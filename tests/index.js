...
describe('mongoose-paginate', function () {
  ...
  it('first page with page and limit', function() {
   ...
   it('test search by filtering the title', function() {
    var query = {
      title: {
        $in: [/Book/i],
      },
    ...
    it('test sort by column', function() {
    var query = {
      title: {
        $in: [/Book/i],
      },
    ...
    var options = {
     ...
     return Book.paginate(query, options).then(result => {
      expect(result.docs[0].price).to.equal(6)
      expect(result.totalDocs).to.equal(100);
      expect(result.limit).to.equal(10);
      expect(result.page).to.equal(1);
      expect(result.pagingCounter).to.equal(1);
    ...
    it('test sort by column desc', function() {
    var query = {
      title: {
        $in: [/Book/i],
      },
    ...
    var options = {
      ...
      return Book.paginate(query, options).then(result => {
      expect(result.docs[0].price).to.equal(600)
      expect(result.totalDocs).to.equal(100);
      expect(result.limit).to.equal(10);
      expect(result.page).to.equal(1);
      expect(result.pagingCounter).to.equal(1);
